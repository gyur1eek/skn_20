...
# --- [추가] DB 연결 및 드라이버 종료를 보장하기 위한 try...finally 구조 ---
try:
    driver.get(url)
    driver.maximize_window()
    time.sleep(5)

    # --- [추가] DB 연결 및 region_id 준비 (while 루프 시작 전) ---
    sido_name = '서울'
    conn = mysql.connector.connect(**db_config)
    cursor = conn.cursor()
    print(f"✅ MySQL '{db_config['database']}' DB에 성공적으로 연결되었습니다!")
    
    cursor.execute("SELECT region_id, sigungu FROM region WHERE sido = %s", (sido_name,))
    region_records = cursor.fetchall()
    region_map = {sigungu: region_id for region_id, sigungu in region_records}
    ...
    while True :
        ...
        print(f"\n... [{sido_name}] {cur_year}년 {cur_month}월 데이터 처리 중 ...")
